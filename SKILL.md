...
version: 2.1.0
description: "通用附件转飞书文档（纯本地版）：将 PDF、PPT、Word、视频、音频、图片、Excel、飞书表格/多维表格等附件转换为高质量排版的飞书云文档。所有处理在本地完成，无需远程服务。当用户说「转成文档」「帮我把这个附件转成飞书文档」「把 PDF/PPT/视频 转成云文档」「解析这个文件」「附件转写」时触发。"
metadata:
  requires:
    bins: ["ffmpeg", "python3"]
...
**关键约束**: 图片插入只能追加到文档**末尾**。必须按 "写文字 -> 插图 -> 写文字 -> 插图" 的顺序交替操作。

#### 步骤:

1. **预处理**: 运行 `render-pdf-pages.py` 将 PDF 渲染为逐页截图。
2. **创建文档 + 首段文字**: 通过飞书 CLI 创建文档，写入标题、概述（用 callout）和第 1 页分析。
...
3. **插入截图**: 通过飞书 CLI 的媒体插入命令，将 `page_001.png` 居中插入。
4. **交替追加文字和图片**: 重复步骤 2-3 直到所有页面处理完毕。
