...
```bash
curl -fsS 'https://file2doc.solutionsuite.cn/skills/file2doc-http/version.json?installed_version=0.1.23'
```

生成最终文档时，Skill 会要求 Agent 围绕用户目标重组内容、清理解析噪声、使用清晰标题层级，并把有价值的页面图或视频帧放在对应步骤附近。最终文档必须包含可追溯的“来源”章节和交付前质量检查。
